	...
	(byteSum % 256) > 0 and(byteSum % 256) == (checkSum + 2))
{
	...
	fltTFS = 0;
	segVal = 0;
	if (SeeLevels[tankNum][tankBaseSeg[tankNum]] == 0)
	{												 // tankBaseSeg[] is 6 for tank 1 and 2 and 9 for tank 0
	...
	else
	{
		...
		if (tankNum == 0)
		{ // Fresh water tank (tank 0) has 8 segment sensor
			for (i = 7; i > (tankFillSeg[tankNum] - 2); i--)
			{ // Add together the percent fill of each segment but the top one
				baseSum = baseSum + tankSegVol8[i];
			}
		}
		else
		{ // Gray and Black tanks (tanks 1 and 2) have 5 segment sensorS
			for (i = 4; i > (tankFillSeg[tankNum] - 2); i--)
			{ // Add together the percent fill of each segment but the top one
		...
		{													   // Calculate the percent fill of the top non-zero segment(gray & black tanks)
	...
	if (tankLev[tankNum] > 100)
		tankLev[tankNum] = 100; // Don't let tankLev exceed 100%
	...
	// Serial.println(" ");
	for (i = 2; i < 10; i++)
	{ // Save and Print read values for verification
...
if (tankNum != 2)
{ // increment or reset tank number
	++tankNum;
}
else
{
...
{								  // passed variable (t) is 0, 1 or 2          //Could use time check instead iof delay?
...
byte readByte()
{ // Function to read individual bytes from SeeLevel sensor
	result = 0;
	for (bitLoop = 0; bitLoop < 8; bitLoop++)
	{										 // We need to populate byte from right to left,
		...
		if (pulseTime <= 26)
		{				 // "0" bit is low for about 43 microseconds, a "1" for about 13 microseconds
